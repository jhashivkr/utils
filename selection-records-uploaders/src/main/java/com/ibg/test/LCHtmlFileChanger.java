...
public class LCHtmlFileChanger {
	...
	public void startLoadProcess() {
		...
		try {
			String htmlFileName = prop.getLcDataHome() + "\\html\\lc_rearranged_2nd_level.html";
	...
	public static void main(String[] args) {
		new LCHtmlFileChanger().startLoadProcess();
	}
				...
				while ((sCurrentLine = br.readLine()) != null) {
					...
					try {
						...
						if (optionPattern.matcher(sCurrentLine).find() && !noOptionPattern.matcher(sCurrentLine).find()) {
							...
							//logic for 3rd level
							//val += "Z";
							//sb.append("<option value=\"").append(val).append("\">").append(sCurrentLine.substring(end + 1)).append('\n');
